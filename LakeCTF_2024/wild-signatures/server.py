...
- Ta có: sigs = public_key + signature
- Cần phải làm sao để to_verif = sig để khi parse_and_vfy_sig(to_verif, msg)
thì nó sẽ in ra "it's valid"
- to_verif = sig <=> user_msg = b""
Hoặc user_msg là tiền tố của sig. (từ dòng 46)

Vậy ta chỉ cần bruteforce từng byte đầu tiên của sig để tìm ra user_msg.
Code solve chạy khá lâu mới ra flag ~30p.
"""