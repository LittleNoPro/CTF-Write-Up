...
class Feistel:
    ...
    def encrypt(self, pt: bytes) -> bytes:
        ...
        for i in range(0, len(pt), self.block_size):
            ct += self._encrypt_block(self.xor(pt[i:i + self.block_size], ct[-self.block_size:]))
        return ct
    ...
    def decrypt(self, ct: bytes) -> bytes:
        ...
        pt = b""
        for i in range(0, len(ct) - self.block_size, self.block_size):
            pt += self.xor(self._decrypt_block(ct[i + self.block_size:i + self.block_size * 2]), ct[i:i + self.block_size])
        return self._unpad(pt, self.block_size)
...
print("Congrats, here is flag HCMUS-CTF{redact}")
